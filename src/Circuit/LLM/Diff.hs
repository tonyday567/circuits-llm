...
import Numeric.LinearAlgebra
  ( Matrix, Vector, cmap, cols, fromList, fromLists, fromRows, maxElement
  , reshape, rows, scale, sumElements, toList, toRows, tr, (|||)
  )
...
splitP (DiffP f1 b1) (DiffP f2 b2) = DiffP
...
joinP (DiffP f1 b1) (DiffP f2 b2) = DiffP
...
multiHeadAttentionP ::
  Int -> Int -> Double -> Matrix Double
  -> DiffP (Matrix Double, Matrix Double, Matrix Double, Matrix Double)
            (Matrix Double) (Matrix Double)
multiHeadAttentionP nHead seqLen _eps mask = DiffP
  { forwardP = \(wq, wk, wv, wo) x ->
      let hDim = cols wq `div` nHead
          heads = [ let wQh = subMatrixW wq 0 (h*hDim) (rows wq) hDim
                        wKh = subMatrixW wk 0 (h*hDim) (rows wk) hDim
                        wVh = subMatrixW wv 0 (h*hDim) (rows wv) hDim
                        q = x LA.<> wQh; k = x LA.<> wKh; v = x LA.<> wVh
      ...
      let hDim = cols wq `div` nHead
          ...
          headFwd h =
            let wQh = subMatrixW wq 0 (h*hDim) (rows wq) hDim
                wKh = subMatrixW wk 0 (h*hDim) (rows wk) hDim
                wVh = subMatrixW wv 0 (h*hDim) (rows wv) hDim
                q = an LA.<> wQh; k = an LA.<> wKh; v = an LA.<> wVh
...
blockParamsFromBlock tb =
  let ff = tbFfn tb
  in  BlockParams
        { bpAttnWq = tbAttnWq tb, bpAttnWk = tbAttnWk tb
        , bpAttnWv = tbAttnWv tb, bpAttnWo = tbAttnWo tb
        , bpAttnLnGamma = tbAttnLnGamma tb, bpAttnLnBeta = tbAttnLnBeta tb
        , bpFfnW1 = ffW1 ff, bpFfnB1 = ffB1 ff
        , bpFfnW2 = ffW2 ff, bpFfnB2 = ffB2 ff
        , bpFfnLnGamma = tbFfnLnGamma tb, bpFfnLnBeta = tbFfnLnBeta tb
        }
...
blockDiffP nHead seqLen eps mask = DiffP
  { forwardP = \p x ->
      let attnOut = forwardP (multiHeadAttentionP nHead seqLen eps mask)
                       (bpAttnWq p, bpAttnWk p, bpAttnWv p, bpAttnWo p) x
          postAttn = x + attnOut
  ...
  , backwardP = \p x dy ->
      ...
      let attnOut = forwardP (multiHeadAttentionP nHead seqLen eps mask)
          ...
          (dXfromAttn, (dgWq, dgWk, dgWv, dgWo)) =
            backwardP (multiHeadAttentionP nHead seqLen eps mask)
              (bpAttnWq p, bpAttnWk p, bpAttnWv p, bpAttnWo p) x dPostAttn
          dX = dPostAttn + dXfromAttn
      in  (dX, BlockParams
             { bpAttnWq = dgWq, bpAttnWk = dgWk, bpAttnWv = dgWv, bpAttnWo = dgWo
             , bpAttnLnGamma = dgAttnGamma, bpAttnLnBeta = dgAttnBeta
             , bpFfnW1 = dgFfnW1, bpFfnB1 = dgFfnB1
             , bpFfnW2 = dgFfnW2, bpFfnB2 = dgFfnB2
             , bpFfnLnGamma = dgFfnGamma, bpFfnLnBeta = dgFfnBeta
             })
  }
...
gptParamsFromModel m = GptParams
  ...
  , backwardP = \p x dy ->
      let fwdStack = foldl' (\acc bp -> acc ++ [applyBlock (last acc) bp]) [x] (gpBlocks p)
          ...
          (gradX0, blockGradsRev) =
            goBwd gradXBlocks (reverse (gpBlocks p)) (reverse (init fwdStack))
      in  (gradX0, GptParams
             { gpBlocks = reverse blockGradsRev
             , gpLnGamma = gLnGamma, gpLnBeta = gLnBeta
             , gpHead = gHead, gpHeadB = gHeadB
             })
  }
  where
    nHead = gptNHead cfg
...
layerNormF x gamma beta eps =
  ...
  in  fromRows [ fromList $ zipWith3 (\v g b -> v * g + b) (toList row) (toList gamma) (toList beta)
               | row <- toRows xHat ]
