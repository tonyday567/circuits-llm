...
import Numeric.LinearAlgebra
  ( Matrix, Vector, cmap, fromList, reshape, rows, cols, scale
...
adamwStep ::
  ...
  -> GptConfig -> AdamWState -> Gpt -> GptGrads
  -> (AdamWState, Gpt)
...
squareGrads g =
  ...
  in  if not ok then error "squareGrads: empty Wte"
      else g
        { ggWte = ggWte g * ggWte g, ggWpe = ggWpe g * ggWpe g
        , ggBlocks = map squareBlockGrads (ggBlocks g)
        , ggLnGamma = ggLnGamma g * ggLnGamma g, ggLnBeta = ggLnBeta g * ggLnBeta g
        , ggHead = ggHead g * ggHead g, ggHeadB = ggHeadB g * ggHeadB g
        }

squareBlockGrads :: BlockGrads -> BlockGrads
squareBlockGrads b = b
  { bgAttnWq = bgAttnWq b * bgAttnWq b, bgAttnWk = bgAttnWk b * bgAttnWk b
  , bgAttnWv = bgAttnWv b * bgAttnWv b, bgAttnWo = bgAttnWo b * bgAttnWo b
  , bgAttnLnGamma = bgAttnLnGamma b * bgAttnLnGamma b
  , bgAttnLnBeta  = bgAttnLnBeta b * bgAttnLnBeta b
  , bgFfnW1 = bgFfnW1 b * bgFfnW1 b, bgFfnB1 = bgFfnB1 b * bgFfnB1 b
  , bgFfnW2 = bgFfnW2 b * bgFfnW2 b, bgFfnB2 = bgFfnB2 b * bgFfnB2 b
...
applyUpdates lr_t eps wd_lr cfg model m v =
  let m_ggWte = ggWte m; v_ggWte = ggWte v
      m_ggWpe = ggWpe m; v_ggWpe = ggWpe v
  in  model
        ...
        , gptBlocks = zipWith3 (updateBlock lr_t eps wd_lr)
                        (gptBlocks model) (ggBlocks m) (ggBlocks v)
...
updateBlock lr_t eps wd_lr tb m v = tb
  ...
  , tbFfn = let ff = tbFfn tb; mf = m; vf = v
            in  ff { ffW1 = updateMatrix lr_t eps wd_lr (ffW1 ff) (bgFfnW1 mf) (bgFfnW1 vf)
...
updateMatrix lr_t eps wd_lr param m_ v_ =
  let nParams = rows param * cols param
      nM = rows m_ * cols m_; nV = rows v_ * cols v_
  in  if nParams /= nM || nParams /= nV
        then error $ "updateMatrix: param=" ++ show (rows param, cols param)
                   ++ " m=" ++ show (rows m_, cols m_) ++ " v=" ++ show (rows v_, cols v_)
        else
          let pv = toList (LA.flatten param)
              mv = toList (LA.flatten m_)
              vv = toList (LA.flatten v_)
          in  reshape (cols param) (fromList $
...
updateVector lr_t eps wd_lr param m_ v_ =
  let pv = toList param; mv = toList m_; vv = toList v_
      np = length pv; nm = length mv; nv = length vv
  in  if np /= nm || np /= nv
...
trainLoop ::
  Double -> Double -> Double -> Double -> Double
  -> GptConfig -> Gpt -> [Int] -> Int -> Int -> IO (Gpt, [Double])
