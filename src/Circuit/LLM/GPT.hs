...
import Numeric.LinearAlgebra
  ( Matrix, Vector, (|||), cmap, cols, fromLists, fromRows
  , konst, rows, scale, sumElements, toList, toRows, tr
  , subMatrix
  )
...
gelu x = cmap f x
  where
    f v = let x' = 1.59577 * v * (1 + 0.044715 * v * v)
          in  v / (1 + exp (-x'))
...
multiHeadAttention nHead x wQ wK wV wO mask =
  let headDim = cols wQ `div` nHead
      ...
      heads = [ scaledDotProductAttention (x LA.<> wSlice wQ h) (x LA.<> wSlice wK h) (x LA.<> wSlice wV h) mask
              | h <- [0 .. nHead - 1] ]
...
transformerBlock nHead tb x =
  let attnOut = multiHeadAttention nHead x
        (tbAttnWq tb) (tbAttnWk tb) (tbAttnWv tb) (tbAttnWo tb)
        (causalMask (rows x))
      attnRes = x + attnOut
...
forward cfg m inputIds =
  let seqLen = length inputIds
      ...
      x = tokEmb + posEmb
      x' = foldl (\acc blk -> transformerBlock (gptNHead cfg) blk acc) x (gptBlocks m)
