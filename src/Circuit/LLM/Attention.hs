...
import qualified Harpie.Array as H (imap)
import Prelude hiding (drop, sum, take, zipWith, maximum)
...
softmax x =
  let rowMaxes = reduces [0] maximum x
      expandedMax = expand (\v _ -> v) rowMaxes (array [nCols] unitVals)
...
multiHeadAttention nHead x wQ wK wV wO mask =
  let [seqLen, nEmbd] = V.toList (shape x)
      ...
      -- Merge: [seq_len, n_embd] and output projection
  in  mult (mergeHeads attnH) wO
